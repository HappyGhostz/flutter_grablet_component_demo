...
import 'package:grab_business_four/four_home_page/page.dart';
...
class FourGrabLet extends GrabLet<String> {
  @override
  FlowBuilder getFlowBuilder(String coreKit, DeepLink deepLink) {
    ...
    WidgetBuilder result = (_) => FourFlowWidget();
  ...
  List<HomePlugin> getHomePlugins(String coreKit) {
    HomeBottomTabPlugin two = HomeBottomTabPlugin(
        ...
        pageBuilder: (_) => FourHomePage().buildPage(null));
    return [two];
  }

  @override
  Map<String, Page<Object, dynamic>> get routes => {RouterSettingNames.THREE_GRAB_LET_MAIN: FourHomePage()};

  @override
  Widget buildWidget(String path, Map<String, dynamic> params) {
    switch (path) {
      case RouterSettingNames.FOUR_GRAB_LET_MAIN:
        return FourHomePage().buildPage(params);
      default:
        return null;
    }
  }
...
class FourFlowWidget extends StatelessWidget {
  FourFlowWidget({
    Key key,
  }) : super(key: key);
